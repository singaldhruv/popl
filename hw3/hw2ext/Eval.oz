         ...
         [] localvar|ident(V)|InnerStmt then
	       %TODO: Keep bound list for closures implementation
            local TempKey NewEnv in
               ...
               {PushStack NStack semstmt(stmt:InnerStmt env:NewEnv)}
            end
	    ...
	    %Conditional. Check if ident(X) is defined or not. If defined, check if it's a boolean. If it's a boolean, operate accordingly
         [] conditional|ident(X)|S1|S2 then
            if {Value.hasFeature TopEnv X} then
               ...
               [] literal(t) then
                  {PushStack NStack semstmt(stmt:S1 env:TopEnv)}
               [] literal(f) then
                  {PushStack NStack semstmt(stmt:S2 env:TopEnv)}
            ...
            [] record|LabelX|PairsX then
               ...
               local NewEnv in
                  try 
                     ...
                     %%%TODO%%%
                     case Error 
                     of success then  {PushStack NStack semstmt(stmt:S1 env:NewEnv)}
                     else {PushStack NStack semstmt(stmt:S2 env:TopEnv)}
                     end
                  end
               end
            ...
            local FuncEnv in
               ...
               of func(def:proced|Vars|Stmt closure:C) then
                  try FuncEnv = {BindParams Params Vars C TopEnv}
                     {PushStack NStack semstmt(stmt:Stmt env:FuncEnv)} 
...
fun {CanStackUnsuspend Stack}
   ...
   of conditional|ident(X)|S1|S2 then
      if {Value.hasFeature TopEnv X} then true
      else false end
      
   of match|ident(X)|P|S1|S2|nil then
...
fun {CanMStackUnsuspend MultStk}
   case MultStk
   of nil then false
   [] H|T then {CanStackUnsuspend H} orelse {CanMStackUnsuspend T}
   end
end

   

fun {EvalMltTh MultStk SuspendL}
   if MultStk == nil then AllThreadsDone
   else
      local Stack TopSemStmt TopStmt TopEnv NStack in
         Stack = {TopStack MultStk}
         TopSemStmt = {TopStack Stack}
            %TopSemStmt contains the top semantic statement
         TopStmt = TopSemStmt.stmt
         TopEnv = TopSemStmt.env
         ...
         of threadt|S|endt then
            local NewStack in
               ...
               {EvalMltTh {PushStack MultStk NewStack} {PushStack SuspendL false}}
            end
            ...
            local RemStack NextStack NewSuspendL NewMStack in
               if {TopStack SuspendL} == false orelse {CanStackUnsuspend Stack} then
                  ...
                  try
                     ...
                     of accepted then
                            %This thread is over
                        {EvalMltTh {PopStack MultStk} {PopStack SuspendL}}
                     else
